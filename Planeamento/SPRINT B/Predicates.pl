...
% Deve ter em atenção que duas tags sintaticamente
...
userWithXTagsInCommon(X, Tags_Users):-
                    ...
                    allCombination(X,SortedMainTagList, TagsCombination),              %faz combinações de x tags
...
tagsInCommon(X,[H|T],ListOfUsers, TempList,  Tag_Users):-
                   usersWithCommonTags(X,H,ListOfUsers,[],UserListWithCommonTag),      %verifica a presenca de uma determinada combinação de tag numa lista de
                   length(UserListWithCommonTag,Length),                               %verifica o tamanho da lista com os users que possui a combinação de dada tag
                   Length>0,                                                           %se for >0 então:
...
usersWithCommonTags(X,TagCombination, [H|T], TempList, Tag_User):-
                    ...
                    removeDuplicates(MainTagListTemp, MainTagList),                    %remove duplicados da lista de tags
                    sort(MainTagList, SortedMainTagList),                              %ordena alfabeticamente as tags
                    allCombination(X,SortedMainTagList, UserTagsCombination),          %faz combinações de x tags
                    member(TagCombination,UserTagsCombination),!,                      %verifica se a combinação das tags referencia está nas combinações do user
...
% Sugerir conexões com outros utilizadores tendo por base as tags e
% conexões partilhadas (até determinado nível).
...
% Determinar o tamanho da rede de um utilizador (até um determinado nivel)
%
% Parametros:
%     - Orig: Origem
%     - Nivel: Nivel
...
% determinar o caminho mais curto (minimiza o número de ligações) para
...
caminho_maisCurto_FindAll(Orig,Dest,LCam):-
                    ...
                    %get_time(T2),
                    %T is T2-T1,nl,
                    %nl, write('Número de soluções encontradas: '),write(Count),
...
caminho_maisCurto(Orig,Dest,LCaminho_minlig):-
                    ...
                    get_time(Tf),
                    T is Tf-Ti,
                    nl, write('Número de soluções encontradas: '),write(NS1),
                    nl, write('Tempo de geracao da solucao:'),write(T),nl.
...
% Consultar o caminho mais seguro (garante que não há uma força de
% ligação inferior a x considerando as forças nos dois sentidos da
% ligação) para determinado utilizador
...
% Lista com todas as conexões de um determinado user
% ---------------------------------------------------
%
% Nota:
...
%
% Nota:
%     - isConnected: unidirecional + única solucao
%     - isConnected2: unidirecional + multiplas solucoes
%     - isConnected3: bidirecionalidade + múltiplas solucoes
...
% O predicado uniao é diferente da concatenação na medida que as
...
% O predicado Interceçao é diferente da uniao na medida que representa
% os valores em comum entre 2 listas (sem repetições).
